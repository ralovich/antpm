...
bool
GarminPacketIntf::interpret(const int lastPid)
{
  //return interpret(lastPid, bytes);
  return tryDecodeDirect(lastPid, bytes);
}


bool
GarminPacketIntf::interpret(const int lastPid, std::vector<uint8_t> data)
{
  size_t off = 0;
  ...
  else
  {
    ...
    sstr << "pid=0x" << toString<int>((int)gp->mPacketId, 4, '0') << "=" << (int)gp->mPacketId << endl;
    ...
    switch(lastPid)
    {
    case L000_Pid_Product_Rqst: // 254
    {
...
#pragma pack(pop)
      ...
      for(size_t i = 4; i+2 < data.size(); i+=3)
      {
        ...
        {
          //sstr << "tag=0x" << toString((int)prota->tag) << "=" << prota->tag << ", data=0x" << toString(prota->data,4,'0') << "=" << toStringDec((int)prota->data,3,'0') << endl;
          std::stringstream sstr2;
          sstr2 << prota->tag << toStringDec((int)prota->data,3,'0');
          caps.push_back(sstr2.str());
        }
        else
          continue;
      }
      for(vector<string>::iterator i = caps.begin(); i!= caps.end(); i++)
      {
        //sstr << *i << endl;
        protos.push_back(*i);
      }
    ...
    case A010_Cmnd_Transfer_Runs:
    {
      uint16_t runs = data[5]<<8 | data[4];
      sstr << "FOUND " << runs << " RUNS!";
      break;
    }
    default:
    {
  ...
  LOG(antpm::LOG_INF) << sstr.str();

  return true;
}

bool
GarminPacketIntf::tryDecodeDirect(const int lastPid, std::vector<uint8_t>& burstData)
{
  ...
  if(beac->beaconId==ANTFS_BeaconId)
  {
    ...
    bool rv = interpret(lastPid, data);
    return rv;
  }
  else if(cmd->commandId==ANTFS_CommandResponseId)
  {
    ...
    //this->chanLastCmd[lastPid] = gpi.interpretPid(data);
    LOG_VAR(interpretPid(data));

    return interpret(-1, data);
  }
  else
  {
...
bool
GarminPacketIntf::saveToFile(const char* fileName /* = "antfs.bin" */)
{
...
bool
GarminPacketIntf::saveToFile(string folder, uint64_t code)
{
  return saveToFile((folder+toString(code, 16, '0')+".bin").c_str());
}
